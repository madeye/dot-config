...
use crate::generators::npmrc::generate_npmrc;
use crate::models::npmrc::npmrc_schema;
use crate::parsers::npmrc::parse_npmrc;
use crate::server::file_ops::{read_dotfile, write_dotfile};

#[component]
pub fn NpmrcPage() -> impl IntoView {
    let schema = npmrc_schema();
    ...
    let load_resource = Resource::new(|| (), move |_| {
        ...
        async move {
            match read_dotfile("~/.npmrc".to_string()).await {
                Ok(result) if result.exists => {
                    let state = parse_npmrc(&result.content);
    ...
    let preview_text = Memo::new(move |_| {
        generate_npmrc(&values.get(), &extra_lines.get())
    });
    ...
    let on_save = Callback::new(move |_: ()| {
        let content = generate_npmrc(&values.get_untracked(), &extra_lines.get_untracked());
        leptos::task::spawn_local(async move {
            let _ = write_dotfile("~/.npmrc".to_string(), content).await;
        });
    });
    ...
    let on_import = Callback::new(move |_: ()| {
        ...
        leptos::task::spawn_local(async move {
            if let Ok(result) = read_dotfile("~/.npmrc".to_string()).await {
                if result.exists {
                    let state = parse_npmrc(&result.content);
                    let mut merged = defaults;
                    for (k, v) in state.values {
