...
use crate::generators::editorconfig::generate_editorconfig;
use crate::models::editorconfig::editorconfig_schema;
use crate::parsers::editorconfig::parse_editorconfig;
use crate::server::file_ops::{read_dotfile, write_dotfile};

#[component]
pub fn EditorconfigPage() -> impl IntoView {
    let schema = editorconfig_schema();
    ...
    let load_resource = Resource::new(|| (), move |_| {
        ...
        async move {
            match read_dotfile("~/.editorconfig".to_string()).await {
                Ok(result) if result.exists => {
                    let state = parse_editorconfig(&result.content);
    ...
    let preview_text = Memo::new(move |_| {
        generate_editorconfig(&values.get(), &extra_lines.get())
    });
    ...
    let on_save = Callback::new(move |_: ()| {
        let content = generate_editorconfig(&values.get_untracked(), &extra_lines.get_untracked());
        leptos::task::spawn_local(async move {
            let _ = write_dotfile("~/.editorconfig".to_string(), content).await;
        });
    });
    ...
    let on_import = Callback::new(move |_: ()| {
        ...
        leptos::task::spawn_local(async move {
            if let Ok(result) = read_dotfile("~/.editorconfig".to_string()).await {
                if result.exists {
                    let state = parse_editorconfig(&result.content);
                    let mut merged = defaults;
                    for (k, v) in state.values {
