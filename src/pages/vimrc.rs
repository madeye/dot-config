...
use crate::generators::vimrc::generate_vimrc;
use crate::models::vimrc::vimrc_schema;
use crate::parsers::vimrc::parse_vimrc;
use crate::server::file_ops::{read_dotfile, write_dotfile};

#[component]
pub fn VimrcPage() -> impl IntoView {
    let schema = vimrc_schema();
    ...
    let load_resource = Resource::new(|| (), move |_| {
        ...
        async move {
            match read_dotfile("~/.vimrc".to_string()).await {
                Ok(result) if result.exists => {
                    let state = parse_vimrc(&result.content);
    ...
    let preview_text = Memo::new(move |_| {
        generate_vimrc(&values.get(), &extra_lines.get())
    });
    ...
    let on_save = Callback::new(move |_: ()| {
        let content = generate_vimrc(&values.get_untracked(), &extra_lines.get_untracked());
        leptos::task::spawn_local(async move {
            let _ = write_dotfile("~/.vimrc".to_string(), content).await;
        });
    });
    ...
    let on_import = Callback::new(move |_: ()| {
        ...
        leptos::task::spawn_local(async move {
            if let Ok(result) = read_dotfile("~/.vimrc".to_string()).await {
                if result.exists {
                    let state = parse_vimrc(&result.content);
                    let mut merged = defaults;
                    for (k, v) in state.values {
