...
use crate::generators::gitconfig::generate_gitconfig;
use crate::models::gitconfig::gitconfig_schema;
use crate::parsers::gitconfig::parse_gitconfig;
use crate::server::file_ops::{read_dotfile, write_dotfile};

#[component]
pub fn GitconfigPage() -> impl IntoView {
    let schema = gitconfig_schema();
    ...
    let load_resource = Resource::new(|| (), move |_| {
        ...
        async move {
            match read_dotfile("~/.gitconfig".to_string()).await {
                Ok(result) if result.exists => {
                    let state = parse_gitconfig(&result.content);
    ...
    let preview_text = Memo::new(move |_| {
        generate_gitconfig(&values.get(), &extra_lines.get())
    });
    ...
    let on_save = Callback::new(move |_: ()| {
        let content = generate_gitconfig(&values.get_untracked(), &extra_lines.get_untracked());
        leptos::task::spawn_local(async move {
            let _ = write_dotfile("~/.gitconfig".to_string(), content).await;
        });
    });
    ...
    let on_import = Callback::new(move |_: ()| {
        ...
        leptos::task::spawn_local(async move {
            if let Ok(result) = read_dotfile("~/.gitconfig".to_string()).await {
                if result.exists {
                    let state = parse_gitconfig(&result.content);
                    let mut merged = defaults;
                    for (k, v) in state.values {
