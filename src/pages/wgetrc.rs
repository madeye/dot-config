...
use crate::generators::wgetrc::generate_wgetrc;
use crate::models::wgetrc::wgetrc_schema;
use crate::parsers::wgetrc::parse_wgetrc;
use crate::server::file_ops::{read_dotfile, write_dotfile};

#[component]
pub fn WgetrcPage() -> impl IntoView {
    let schema = wgetrc_schema();
    ...
    let load_resource = Resource::new(|| (), move |_| {
        ...
        async move {
            match read_dotfile("~/.wgetrc".to_string()).await {
                Ok(result) if result.exists => {
                    let state = parse_wgetrc(&result.content);
    ...
    let preview_text = Memo::new(move |_| {
        generate_wgetrc(&values.get(), &extra_lines.get())
    });
    ...
    let on_save = Callback::new(move |_: ()| {
        let content = generate_wgetrc(&values.get_untracked(), &extra_lines.get_untracked());
        leptos::task::spawn_local(async move {
            let _ = write_dotfile("~/.wgetrc".to_string(), content).await;
        });
    });
    ...
    let on_import = Callback::new(move |_: ()| {
        ...
        leptos::task::spawn_local(async move {
            if let Ok(result) = read_dotfile("~/.wgetrc".to_string()).await {
                if result.exists {
                    let state = parse_wgetrc(&result.content);
                    let mut merged = defaults;
                    for (k, v) in state.values {
