...
use crate::generators::shellrc::generate_shellrc;
use crate::models::shellrc::shellrc_schema;
use crate::parsers::shellrc::parse_shellrc;
use crate::server::file_ops::{read_dotfile, write_dotfile};

#[component]
pub fn ShellrcPage() -> impl IntoView {
    let schema = shellrc_schema();
    ...
    let load_resource = Resource::new(|| (), move |_| {
        ...
        async move {
            match read_dotfile("~/.bashrc".to_string()).await {
                Ok(result) if result.exists => {
                    let state = parse_shellrc(&result.content);
    ...
    let preview_text = Memo::new(move |_| {
        generate_shellrc(&values.get(), &extra_lines.get())
    });
    ...
    let on_save = Callback::new(move |_: ()| {
        let content = generate_shellrc(&values.get_untracked(), &extra_lines.get_untracked());
        leptos::task::spawn_local(async move {
            let _ = write_dotfile("~/.bashrc".to_string(), content).await;
        });
    });
    ...
    let on_import = Callback::new(move |_: ()| {
        ...
        leptos::task::spawn_local(async move {
            if let Ok(result) = read_dotfile("~/.bashrc".to_string()).await {
                if result.exists {
                    let state = parse_shellrc(&result.content);
                    let mut merged = defaults;
                    for (k, v) in state.values {
