...
use crate::generators::emacs::generate_emacs;
use crate::models::emacs::emacs_schema;
use crate::parsers::emacs::parse_emacs;
use crate::server::file_ops::{read_dotfile, write_dotfile};

#[component]
pub fn EmacsPage() -> impl IntoView {
    let schema = emacs_schema();
    ...
    let load_resource = Resource::new(|| (), move |_| {
        ...
        async move {
            match read_dotfile("~/.emacs".to_string()).await {
                Ok(result) if result.exists => {
                    let state = parse_emacs(&result.content);
    ...
    let preview_text = Memo::new(move |_| {
        generate_emacs(&values.get(), &extra_lines.get())
    });
    ...
    let on_save = Callback::new(move |_: ()| {
        let content = generate_emacs(&values.get_untracked(), &extra_lines.get_untracked());
        leptos::task::spawn_local(async move {
            let _ = write_dotfile("~/.emacs".to_string(), content).await;
        });
    });
    ...
    let on_import = Callback::new(move |_: ()| {
        ...
        leptos::task::spawn_local(async move {
            if let Ok(result) = read_dotfile("~/.emacs".to_string()).await {
                if result.exists {
                    let state = parse_emacs(&result.content);
                    let mut merged = defaults;
                    for (k, v) in state.values {
